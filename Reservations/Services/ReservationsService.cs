namespace Reservations.Services
{
    ...
    public class ReservationsService : IReservationsService
    {
        ...
        public ValidationResult ValidateNewReservation(NewReservationItem newReservation)
        {
            ...
            if (!(newReservation.From.Hour >= 8 && newReservation.From.Hour <= 18
               && newReservation.To.Hour >= 8 && newReservation.To.Hour <= 18))
            {
                result |= ValidationResult.OutsideWorkingHours;
            }

            if ((newReservation.To - newReservation.From).TotalMinutes > 180)
            {
                result |= ValidationResult.TooLong;
            }

            var isThereAnyConflict = _queryAll.Execute().Any(x => x.Hall.Number == newReservation.LectureHallNumber && newReservation.To > x.From && newReservation.To < x.To);
            ...
            var halls = _queryAll.Execute();
            var isHallExists = _queryAllLectureHalls.Execute().Any(x => x.Number == newReservation.LectureHallNumber);
            ...
            var isLecturerExists = _queryAllLecturers.Execute().Any(x => x.Id == newReservation.LecturerId);
            if (!isLecturerExists)
            {
